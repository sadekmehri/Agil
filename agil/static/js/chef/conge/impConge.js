$(document).ready(function() {
    ...
    $("#idForm").submit(function(e) {
        ...
        $.ajax({
            url: "/chef/conge/imprimerListe/data",
            type: 'post',
            ...
            success: function(data, textStatus, xhr) {
                ...
                if (status === 200) {
                    ...
                    setTimeout(function() {
                        $("#myTable tbody").html("");
                        Data(data);
                        success("Récupération réussie des données");
                        $("#load").hide();
                    }, 250);
                } else if (status === 201) {
                    let lgDateDeb = 0;
    ...
    function Data(data) {
        ...
        $.each(data, function(i, d) {
            ...
            $.each(d, function(i, d) {
                Tot+=d.Nbr;
                appendToResult = appendToResult.concat(d.Station + "</td>" +
                    "<td>" + d.Groupe + "</td>" +
                    "<td>" + d.Code + "</td>" +
                    "<td>" + d.Cin + "</td>" +
                    "<td>" + d.Nom + "</td>" +
            ...
            appendToResult = appendToResult.concat("</tr><td colspan='8'></td></tr>");
    ...
    function generate(datDeb,datFin) {
        if(!datDeb){datDeb = currrent_date();}
        if(!datFin){datFin = currrent_date();}
        ...
        const pageContent = function (data) {
            ...
            pdf.setFontSize(14);
            pdf.text(660, 33, datDeb + " -- " + datFin);
            pdf.text(700, 53, 'Liste de congé');
        ...
        pdf.save(datDeb+"_"+datFin+"_Conge"+getStation()+".pdf");
    }

    $this.click(function(e) {
        e.preventDefault();
        $this.attr('disabled', true);
        generate(datDeb,datFin);
    });
