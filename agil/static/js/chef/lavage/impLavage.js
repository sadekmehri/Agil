$(document).ready(function() {
    ...
    $("#idForm").submit(function(e) {
        ...
        const $ErrGrp = $("#ErrGrp span");
        dates = $date.val();
        $ErrDate.text("");
        $ErrGrp.text("");
        const $classDate = $date.attr('class');
        const $classGrp = $grp.attr('class');
        if ($classDate === "form-control is-invalid") {
            $date.removeClass('form-control is-invalid').addClass('form-control');
        }
        if ($classGrp === "form-control is-invalid") {
            $grp.removeClass('form-control is-invalid').addClass('form-control');
        }
        $.ajax({
            url: "/chef/recette/lavage/dataLavage",
            type: 'post',
            ...
            success: function(data, textStatus, xhr) {
                ...
                if (status === 200) {
                    ...
                    $ErrDate.append(str);
                    str = "";
                    for (var i = 0; i < lgGrp; i++) {
                        str += data["data"].Grp[i] + "\n";
                        $grp.removeClass('form-control').addClass('form-control is-invalid');
                    }
                    $ErrGrp.append(str);
                }
            }
    ...
    function Data(data) {
        ...
        $.each(data, function(i, d) {
            ...
            $.each(d, function(i, d) {
                Tot += d.PrixLavage;
                appendToResult = appendToResult.concat(d.DateLavage + "</td>" +
                    "<td>" + d.MatriculeVoiture + "</td>" +
                    "<td>" + d.HeureDebut + " - "+ d.HeureFin +"</td>" +
                    "<td>" + d.TypeLavage + "</td>" +
                    "<td>" + d.Groupe + "</td>" +
                    "<td>" + d.PrixLavage + " TND " + "</td>" +
                    "</tr>"
                );
            });
            ...
            appendToResult = appendToResult.concat(
                ...
                "</tr>" +
                "<td class='bg-dark' colspan='5'>Total</td>" +
    ...
    function generate(dates) {
        ...
        pdf.save(dates +"_"+currrent_date()+"_Recette_Lavage_"+getStation()+".pdf");
    }
