$(document).ready(function() {
    ...
    $("#idForm").submit(function(e) {
        ...
        e.stopPropagation();
        const $list = $("#Mat");
        const $ErrMat = $("#ErrMat span");
        $ErrMat.text("");
        const $classMat = $list.attr('class');
        if ($classMat === "form-control is-invalid") {
            $list.removeClass('form-control is-invalid').addClass('form-control');
        }
        $.ajax({
            url: "/chef/recette/lavage/voiture",
            type: 'post',
            ...
            success: function(data, textStatus, xhr) {
                ...
                if (status === 200) {
                    ...
                    let str = "";
                    if (data["data"].Mat) {
                        lgMat = data["data"].Mat.length;
                    }
                    str = "";
                    for (var i = 0; i < lgMat; i++) {
                        str += data["data"].Mat[i] + "\n";
    ...
    function Data(data) {
        ...
        $.each(data, function(i, d) {
            ...
            $.each(d, function(i, d) {
                Tot += d.PrixLavage;
                appendToResult = appendToResult.concat(d.MatriculeVoiture + "</td>" +
                    "<td>" + d.DateLavage + "</td>" +
                    "<td>" + d.TypeLavage + "</td>" +
                    "<td>" + d.Groupe + "</td>" +
                    "<td>" + d.HeureDebut + "</td>" +
            ...
            appendToResult = appendToResult.concat(
                ...
                "</tr>" +
                "<td class='bg-dark' colspan='6'>Total</td>" +
    ...
    $("#Mat").inputmask({
        "mask": '9{1,3}-TUN-9999',"clearIncomplete": true
    });
    ...
    function generate(dates) {
        ...
        const pageContent = function (data) {
            ...
            pdf.setFontSize(14);
            pdf.text(740, 53, currrent_date());
            pdf.text(733, 33, 'Recu Voiture');
        ...
        pdf.save(dates +"_"+currrent_date()+"_Recu_Voiture_"+getStation()+".pdf");
    }
