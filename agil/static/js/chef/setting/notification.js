   ...
   $('#daterange-ranges').daterangepicker({
       ...
       function(start, end) {
           ...
           loadNotificationData($("#daterange-ranges").data('daterangepicker').startDate.format('YYYY-MM-DD'), $("#daterange-ranges").data('daterangepicker').endDate.format('YYYY-MM-DD'));
       }
   );
   ...
   loadNotificationData($("#daterange-ranges").data('daterangepicker').startDate.format('YYYY-MM-DD'), $("#daterange-ranges").data('daterangepicker').endDate.format('YYYY-MM-DD'));
   function loadNotificationData(startDate,endDate) {
       if(!startDate){ startDate = currrent_date();}
       if(!endDate){ endDate = currrent_date();}
       $.ajax({
           url: '/chef/setting/notification/details',
           type: 'post',
   ...
   function generate_table() {
       ...
       for (let i = 0; i < displayRecords.length; i++) {
           const htmlStr = `<li class='feed-item'>
                       <span class='date'>${displayRecords[i].Subject} -- ${displayRecords[i].Time} </span>
                       <span class='activity-text'>${displayRecords[i].Text}</span>
                       </li>`;
