...
function createChartCarb() {
   ...
   function loadCarb(idStation,startDate,endDate) {
       var don = [];
       if(!startDate){ startDate = currrent_date();}
       if(!endDate){ endDate = currrent_date();}
       if(!idStation){ idStation = 0;}
       $.ajax({
           url: '/administrateur/station/statistique/carburant',
           type: 'post',
       ...
       function(start, end) {
           $('#daterange-ranges1 span').html(start.format('MMMM D') + ' - ' + end.format('MMMM D'));
            ...
            setTimeout(function () {
                $("#loadCarb").hide();
                loadCarb(GetIdStation(),$("#daterange-ranges1").data('daterangepicker').startDate.format('YYYY-MM-DD'),$("#daterange-ranges1").data('daterangepicker').endDate.format('YYYY-MM-DD'));
                $("#carb").show();
            },1000);
       }
       );
